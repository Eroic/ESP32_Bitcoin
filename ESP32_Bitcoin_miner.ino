...
UTFT myGLCD(ILI9325D_8,25,26,27,28);
...
int reply(uint32_t data) {
  ...
  myGLCD.print("out:", 0, hauteur);
  myGLCD.printNumI(data ,32 ,hauteur);
  hauteur += 10;
  if (hauteur == 180) hauteur = 0;
}
...
char *btohhi(char *dest, uint8_t *src, int len) {
  char *d = dest;
  src +=2;
  while( len-- ) sprintf(d, "%02x", (unsigned char)*src++), d += 2;
  return dest;
...
void setup() {
  ...
  //myGLCD.print(btoh(hex, hash, 32),0,10);
  SerialUSB.println(btoh(hex, hash, 32));
#endif
}

void loop() {
  if (SerialUSB.available() > 0) {
    int bytes = SerialUSB.readBytes(buf, size);
    if (bytes>0) {
      uint32_t sig = ntohl(*(uint32_t*)buf);
      myGLCD.print("in:", 0, hauteur);
      myGLCD.printNumI(sig,24,hauteur);
//      myGLCD.print(btoh(hex, (uint8_t*)buf, 4),24,hauteur);
      hauteur += 10;
      if (hauteur == 180) hauteur = 0;
      switch(sig) {
        case golden_ob: {
          reply(golden_nonce);
          //myGLCD.print("out:", 0, hauteur);
          //myGLCD.print(btoh(hex, (uint8_t*)golden_nonce, 4),32,hauteur);
          //hauteur += 10;
          break;
        }
        case workdiv_sig: {
          reply(workdiv1);
          //myGLCD.print("out:", 0, hauteur);
          //myGLCD.print(btoh(hex, (uint8_t*)workdiv1, 4),32,hauteur);
          //hauteur += 10;
          break;
        }
        default:
          ...
          //myGLCD.printNumI(nonce,0,210);
          //myGLCD.print(btoh(hex, hash, 16),0,220);
          //myGLCD.print(btohhi(hex, hash, 16),0,230);
...
//#ifdef DEBUG
            ...
            if (end-start>=1000 || start==-1) {
              start = end;
              if (seconds) {
                myGLCD.printNumI((float)nonce/seconds,0,200);
                myGLCD.print(" hashes/sec.",32,200);
              }
              seconds++;
            }
//#endif
            ...
            if (!midstate_hashing(hash, (uint8_t*)buf, nonce)) {
                reply(nonce);
                myGLCD.print("out:", 0, hauteur);
                myGLCD.printNumI(nonce,32,hauteur);
                hauteur += 10;
                if (hauteur == 180) hauteur = 0;
                myGLCD.print("out:", 0, hauteur);
                myGLCD.print(btoh(hex, hash, 16),32,hauteur);
                hauteur += 10;
                if (hauteur == 180) hauteur = 0;
                myGLCD.print("out:", 0, hauteur);
                myGLCD.print(btohhi(hex, hash, 16),32,hauteur);
                hauteur += 10; 
                if (hauteur == 180) hauteur = 0;
